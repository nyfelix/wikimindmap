...
return nice;
	...
	static public  function get_wiki_link(mc_padre,depth){
    	var link=mc_padre.createEmptyMovieClip("wikiLink",6+depth);
		var color=0xEE0000;
		
		// Drawing the W
		/*link.lineStyle(1,0x606060,100);
		link.moveTo(6,2);
		link.lineTo(8,4);
		link.moveTo(9,5);
		link.lineTo(11,7);
		...
		link.beginFill(0xFFFFFF,100);
		link.lineStyle(0,0x101010,100);
		link.moveTo(0,6);
		link.lineTo(4,2);
		link.lineTo(10,2);
		link.lineTo(16,8);
	...
	static public  function get_mm_link(mc_padre,depth){
    	var link=mc_padre.createEmptyMovieClip("mmLink",6+depth);
		var color=0xEE0000;
		
		link.lineStyle(3,0x009900,50);
		...
		link.moveTo(x+Math.cos(anchorAngle)*radius, y+Math.sin(anchorAngle)*radius);
		for (var i=0; i<accuracy; ++i) {
			controlAngle = anchorAngle+span;
			anchorAngle = controlAngle+span;
			if (i != 4 && i!=5 && i!=0 && i!=9) {
				link.curveTo(   x + Math.cos(controlAngle)*controlRadius,
				...
				link.moveTo(x + Math.cos(anchorAngle)*radius,
							y + Math.sin(anchorAngle)*radius);
			}
	...
	static public  function get_web_link(mc_padre,depth){
    	var link=mc_padre.createEmptyMovieClip("webLink",6+depth);
		var color=0xEE0000;

		link.beginFill(0xFFFFFF,100);
		link.lineStyle(1,0x0000FF,100);
		link.moveTo(1,15);
		link.lineTo(1,6);
		link.lineTo(10,6);
		link.lineTo(10,15);
	...
	static public  function genLink(mc_padre,depth,color){
		...
		link.moveTo(2,6);
		loadMovie("doc.jpg",link);
		return link;
	}

	static public  function get_internlink(mc_padre,depth){ //butterfly
	...
	static public  function get_weblink(mc_padre,depth){ //butterfly
		var butterfly=mc_padre.createEmptyMovieClip("butterfly",6+depth);
		var color=0xEE0000;
	...
	static public  function get_wikilink(mc_padre,depth){ //butterfly
		var butterfly=mc_padre.createEmptyMovieClip("butterfly",6+depth);
		var color=0xEE0000;
